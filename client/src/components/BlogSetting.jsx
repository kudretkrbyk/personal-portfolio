...
export default function BlogSetting() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.image) form.append("image", formData.image); // opsiyonel kontrol

    try {
      if (editMode) {
        await updateBlog({ id: editId, formData: form }).unwrap(); // 👈 burada "formData: form"
      } else {
        console.log("prpoje ekleme", form);
